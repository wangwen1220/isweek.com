...
class AssetController extends AdminbaseController {
     ...
     */
	//Ueditor图片上传
	public function swfupload_multi(){
        if (IS_POST) {
			$config = array(
            ...
            if (!empty($info)) {
				...
				$state = 'SUCCESS';
				$file = C("UPLOADPATH").$info['upfile']['savepath'].$info['upfile']['savename'];
    ...
    public function swfupload() {
        if (IS_POST) {
            ...
            if (!empty($info)) {
                //上传成功
                //写入附件数据库信息
				echo "1," . C("UPLOADPATH").$info['Filedata']['savepath'].$info['Filedata']['savename'].",".'1,'.$info['Filedata']['name'];
				exit;
            } else {
                //上传失败，返回错误
    ...
    protected function att_not_used() {
        ...
        $att_json = \Org\Cmf\Input::getVar(cookie('att_json'));
        if ($att_json) {
            if ($att_json) {
