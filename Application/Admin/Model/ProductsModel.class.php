...
class ProductsModel extends Model {
	...
	public function deleteCategory($cid) {
		//删除所有产品中的该分类
		$sql = "update sp_products set categorys=if(replace(categorys, ',$cid,', ',')=',', '',replace(categorys, ',$cid,', ','))";
		$this->execute($sql);
	}
	...
	public function getCategoryByIds($categorys) {
		$return = array();
		$cate_ids = trim($categorys, ',');
		$terms_obj = M('Terms');
		$map['term_id'] = array('IN', $cate_ids);
		$return = $terms_obj->where($map)->select();
		return $return;
	}
	public function getSupplierByIds($categorys) {
		$return = array();
		$cate_ids = trim($categorys, ',');
		$terms_obj = M('Suppliers');
