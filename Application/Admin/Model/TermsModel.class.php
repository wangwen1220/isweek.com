...
//			array('name', 'require', '分类名称不能为空！', 1, 'regex', 3),
	...
	public function getTerm($category = null, $pid = null, $status = 1) {
		...
		$data = $this->field ( "*, concat(path,'-',term_id) as bpath" )->where ( $where )->order ( "name asc, listorder asc" )->select ();
//		echo $this->getLastSql();
		foreach ( $data as $key => $val ) {
			$data [$key] ['level'] = substr_count ( $val ['path'], '-' );
		}
		return $data;
	
	}

	public function terms_cache() {//分类缓存
		$data = $this->getTerm();
		F("terms_cache", $data);
	}
	...
	public function updateProductsCount($term_array, $increase=true) {
/*
		foreach ($term_array as $v) {
			$current = $v;
			//此种递归算法
			...
			do {
				...
				if($increase) {
					echo $current.'+';
//					$this->where('term_id='.$v)->setInc('count');
				} else {
					echo $current.'-';
//					$this->where('term_id='.$v)->setDec('count');
				}
				//获取父级分类
				$term_data = $this->getTermByID($current);
				$current = $term_data['parent'];
			}while($current > 0) ;
			 
			//根据数据逻辑，保留了分类路径
			$term_data = $this->getTermByID($current);
			$update_termid = array_filter(explode('-', $term_data['path']));
			$map['term_id'] = array('IN', $update_termid);
			if($increase) {
...
*/
		...
		$total_terms = array();
		foreach ($term_array as $v) {
			$term_data = $this->getTermByID($v);
			$update_termid = array_filter(explode('-', $term_data['path']));
			$total_terms = array_merge($total_terms, $update_termid);
		}
		$unique_terms = array_unique($total_terms);
		$map['term_id'] = array('IN', $unique_terms);
		if($increase) {
			$this->where($map)->setInc('count');
		} else {
			$this->where($map)->setDec('count');
	...
	public function get_terms_path($term_id) {
		$return = "";
		$term_data = $this->getTermByID($term_id);
		$term_path = array_filter(explode('-', $term_data['path']));
		$flag = "";
		foreach($term_path as $v) {
			$tmp = $this->getTermByID($v);
